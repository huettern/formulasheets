...
% @Last Modified time: 2019-11-07 14:30:20
% ---------------------------------------------------------------------------
...
\begin{multicols*}{4}
  ...
  The definition of information $l$ is:
  \begin{empheq}[box=\eqbox]{gather*}
    l(s_k) \triangleq -\log p_k
  \end{empheq}
  \begin{empheq}{align*}
    &l(s_k) = 0 &&\text{for} \quad p_k=1 \\
    &l(s_k) \geq 0 &&\text{for} \quad 0\leq p_k \leq 1 \\
    &l(s_k) > l(s_i) &&\text{for} \quad p_k < p_i \\
    &l(s_ks_i)= l(s_k) + l(s_i) &&\text{if stat. indep.} \quad p_{ki}=p_kp_i
  \end{empheq}

  When $l$ is specified in bits, logarithms are to base 2.

  The definition of entropy $H$ is:
  \begin{empheq}[box=\eqbox]{gather*}
    H(\mathcal{S}) \triangleq E[l(\mathcal{S})] = \sum_{k=0}^{K-1}p_kl(s_k) = -\sum_{k=0}^{K-1}p_k\log p_k
  \end{empheq}
  \begin{empheq}{align*}
