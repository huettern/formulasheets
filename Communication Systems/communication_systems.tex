...
% @Last Modified time: 2019-11-14 10:49:03
% ---------------------------------------------------------------------------
...
\begin{multicols*}{4}
  ...
  \textbf{Mutual information} is the reductino of the uncertainty about $X$ achieved by observing $Y$.
  \begin{empheq}[box=\eqbox]{gather*}
  ...
  $p(x)$.
  \begin{empheq}{gather*}
    Y(X;Y) = H(Y)-H(Y|X) = \\\sum_{k=0}^{K-1}\sum_{j=0}^{J-1}p(x_j,y_k)\log\left(\frac{ p(y_k|x_j)}{p(y_k)}\right)=\\
